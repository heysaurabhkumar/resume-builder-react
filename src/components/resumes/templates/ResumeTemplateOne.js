import React from "react";

export default function ResumeTemplateOne() {
  return (
    <>
      <div id="resume" className="container mt-3">
        <div className="row">
          <div className="col-md-5 py-5 bgleft">
            <div className="text-center">
              <img
                src="data:image/png;base64,"
                alt="profile"
                className="img-fluid myimg mb-2"
              />
              <p className="text-center display-1 fw-bolder">Saurabh Kumar</p>
              <p>Associate Software Developer</p>
            </div>
            <p className="text-break">
              <strong>Email:</strong> thesaurabh4714@gmail.com
            </p>
            <p>
              <strong>Contact No.:</strong> 7004960935
            </p>
            <p className="new-line">
              <strong>Address:</strong> Room No. 23, Om Pg, Law gate, Jalandhar,
              Punjab - 144401
            </p>

            <hr />

            <h4>Objective</h4>

            <p>
              Lorem ipsum dolor sit amet consectetur, adipisicing elit. Deleniti
              tenetur atque impedit ipsum quibusdam magnam, similique ratione
              fuga, dolorum accusamus reprehenderit cum minima beatae nostrum
              aut quod modi corporis et!
            </p>

            <hr />
            <h4>Skills</h4>
            <ul>
              <li>JavaScript</li>
              <li>JavaScript</li>
              <li>JavaScript</li>
              <li>JavaScript</li>
              <li>JavaScript</li>
            </ul>
            <hr />

            <h4>Social Links</h4>
            <div className="text-break">
              <p>
                <strong>LinkedIn:</strong> linkedin.com
              </p>
              <p>
                <strong>Facebook:</strong> facebook.com
              </p>
              <p>
                <strong>Instagram:</strong> instagram.com
              </p>
            </div>
            <hr />

            <h4>Languages</h4>
            <ul>
              <li>English</li>
              <li>Hindi</li>
            </ul>
          </div>
          <div className="col-md-7 py-5 bgright">
            <div className="card mt-4">
              <div className="card-header header-one">
                <h3>Work Experience</h3>
              </div>
              <div className="card-body body-one">
                <ul>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                    Doloribus eius excepturi natus consectetur amet! Molestiae
                    praesentium aperiam, libero consectetur amet rerum.
                    Consectetur consequuntur numquam obcaecati maiores esse
                    aperiam qui tenetur.
                  </li>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                    Doloribus eius excepturi natus consectetur amet! Molestiae
                    praesentium aperiam, libero consectetur amet rerum.
                    Consectetur consequuntur numquam obcaecati maiores esse
                    aperiam qui tenetur.
                  </li>
                </ul>
              </div>
            </div>

            <div className="card mt-4">
              <div className="card-header header-one">
                <h3>Projects</h3>
              </div>
              <div className="card-body body-one">
                <ul>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                    Doloribus eius excepturi natus consectetur amet! Molestiae
                    praesentium aperiam, libero consectetur amet rerum.
                    Consectetur consequuntur numquam obcaecati maiores esse
                    aperiam qui tenetur.
                  </li>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                    Doloribus eius excepturi natus consectetur amet! Molestiae
                    praesentium aperiam, libero consectetur amet rerum.
                    Consectetur consequuntur numquam obcaecati maiores esse
                    aperiam qui tenetur.
                  </li>
                </ul>
              </div>
            </div>

            <div className="card mt-4">
              <div className="card-header header-one">
                <h3>Certifications</h3>
              </div>
              <div className="card-body body-one">
                <ul>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                    Doloribus eius excepturi natus consectetur amet! Molestiae
                    praesentium aperiam, libero consectetur amet rerum.
                    Consectetur consequuntur numquam obcaecati maiores esse
                    aperiam qui tenetur.
                  </li>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                    Doloribus eius excepturi natus consectetur amet! Molestiae
                    praesentium aperiam, libero consectetur amet rerum.
                    Consectetur consequuntur numquam obcaecati maiores esse
                    aperiam qui tenetur.
                  </li>
                </ul>
              </div>
            </div>

            <div className="card mt-4">
              <div className="card-header header-one">
                <h3>Education</h3>
              </div>
              <div className="card-body body-one">
                <ul>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                    Doloribus eius excepturi natus consectetur amet! Molestiae
                    praesentium aperiam, libero consectetur amet rerum.
                    Consectetur consequuntur numquam obcaecati maiores esse
                    aperiam qui tenetur.
                  </li>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                    Doloribus eius excepturi natus consectetur amet! Molestiae
                    praesentium aperiam, libero consectetur amet rerum.
                    Consectetur consequuntur numquam obcaecati maiores esse
                    aperiam qui tenetur.
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="container text-center mb-3">
        <button className="btn btn-primary btn-lg">Download</button>
      </div>
    </>
  );
}
