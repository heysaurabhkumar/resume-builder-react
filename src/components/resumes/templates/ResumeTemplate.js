import React from "react";

export default function ResumeTemplate() {
  return (
    <>
      <div id="resume" className="container body mt-3">
        <div className="row">
          <div className="col-md-3 py-5 background">
            <div className="text-center">
              <img
                src="data:image/png;base64,"
                alt="profile"
                className="img-fluid myimg mb-2"
              />

              <p>Associate Software Developer</p>
            </div>
            <p className="text-break">
              <strong>Email:</strong> thesaurabh4714@gmail.com
            </p>
            <p>
              <strong>Contact No.:</strong> 7004960935
            </p>
            <p className="new-line">
              <strong>Address:</strong> Room No. 23, Om Pg, Law gate, Jalandhar,
              Punjab - 144401
            </p>

            <hr />
            <h4>Skills</h4>
            <ul>
              <li>Python</li>
              <li>Python</li>
              <li>Python</li>
              <li>Python</li>
              <li>Python</li>
              <li>Python</li>
              <li>Python</li>
            </ul>
            <hr />

            <h4>Social Links</h4>
            <div className="text-break">
              <p>
                <strong>LinkedIn:</strong> linked.com
              </p>
              <p>
                <strong>Facebook:</strong> facebook.com
              </p>
              <p>
                <strong>Instagram:</strong> instagram.com
              </p>
            </div>
            <hr />

            <h4>Languages</h4>
            <ul>
              <li>Hindi</li>
              <li>English</li>
            </ul>
          </div>
          <div className="col-md-9 py-5">
            <p className="text-center display-1 fw-bolder">Saurabh Kumar</p>
            <div className="card mt-4">
              <div className="card-header background">
                <h3>Objective</h3>
              </div>
              <div className="card-body">
                <p>
                  Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                  Deleniti, accusamus cupiditate! Amet, corporis illum officia
                  voluptate ut minus dolor, enim alias, deserunt illo cum!
                  Nesciunt incidunt facilis consequatur exercitationem odio.
                </p>
              </div>
            </div>

            <div className="card mt-4">
              <div className="card-header background">
                <h3>Work Experience</h3>
              </div>
              <div className="card-body">
                <ul>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    Voluptate rerum dicta ad fugiat ipsa unde, nobis quibusdam
                    earum atque illum sunt ducimus alias similique aliquid fugit
                    mollitia sed suscipit officia.
                  </li>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    Voluptate rerum dicta ad fugiat ipsa unde, nobis quibusdam
                    earum atque illum sunt ducimus alias similique aliquid fugit
                    mollitia sed suscipit officia.
                  </li>
                </ul>
              </div>
            </div>

            <div className="card mt-4">
              <div className="card-header background">
                <h3>Projects</h3>
              </div>
              <div className="card-body">
                <ul>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    Voluptate rerum dicta ad fugiat ipsa unde, nobis quibusdam
                    earum atque illum sunt ducimus alias similique aliquid fugit
                    mollitia sed suscipit officia.
                  </li>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    Voluptate rerum dicta ad fugiat ipsa unde, nobis quibusdam
                    earum atque illum sunt ducimus alias similique aliquid fugit
                    mollitia sed suscipit officia.
                  </li>
                </ul>
              </div>
            </div>

            <div className="card mt-4">
              <div className="card-header background">
                <h3>Certifications</h3>
              </div>
              <div className="card-body">
                <ul>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    Voluptate rerum dicta ad fugiat ipsa unde, nobis quibusdam
                    earum atque illum sunt ducimus alias similique aliquid fugit
                    mollitia sed suscipit officia.
                  </li>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    Voluptate rerum dicta ad fugiat ipsa unde, nobis quibusdam
                    earum atque illum sunt ducimus alias similique aliquid fugit
                    mollitia sed suscipit officia.
                  </li>
                </ul>
              </div>
            </div>

            <div className="card mt-4">
              <div className="card-header background">
                <h3>Education</h3>
              </div>
              <div className="card-body">
                <ul>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    Voluptate rerum dicta ad fugiat ipsa unde, nobis quibusdam
                    earum atque illum sunt ducimus alias similique aliquid fugit
                    mollitia sed suscipit officia.
                  </li>
                  <li className="new-line mt-2">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    Voluptate rerum dicta ad fugiat ipsa unde, nobis quibusdam
                    earum atque illum sunt ducimus alias similique aliquid fugit
                    mollitia sed suscipit officia.
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="container text-center mb-3">
        <button className="btn btn-primary btn-lg">Download</button>
      </div>
    </>
  );
}
